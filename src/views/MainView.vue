...
document.addEventListener('keyup', function (e) {
	if (e.key === 'r' && !e.shiftKey && !e.ctrlKey && e.altKey && !e.metaKey) {
		refresh();
	} else if (e.key === 'q' && !e.shiftKey && !e.ctrlKey && e.altKey && !e.metaKey) {
		exit();
	}
}, false);
...
<template>
	<el-row class="el-row">
		<el-col :span="12" :offset="6" class="text-input-col">
			<el-input class="el-input" v-model="input" @keyup.enter="open_room(input)" size="large"
				placeholder="在此输入直播间代码"></el-input>
		</el-col>
			...
			<el-tab-pane label="历史" name="second">
				<ul class="card-list">
					<li v-for="(k, i) in history_rooms" :key="i" class="card-list-item">
						<live-room-card-his :cover="room_info.get(k)?.cover ?? ''"
							:title="room_info.get(k)?.title ?? ''"
							:owner="room_info.get(k)?.owner ?? ''"
							:is_living="room_info.get(k)?.is_living ?? false"
							:loading="room_info.get(k)?.is_loading ?? true" :room_code="k"
							@add_saved="add_new_saved(k)" @open_room="open_room(k)"
