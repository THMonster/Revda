function checkUrl(u)
{
    ...
    if (u.includes('bilibili.com')) {
        ...
        url = url.hostname + url.pathname + (url.searchParams.get('v') ? '?v=' + url.searchParams.get('v') : '');
    }
    if (u.includes('bilibili.com/video') ||
        u.includes('bilibili.com/bangumi/play') ||
        u.includes('live.bilibili.com/') ||
        u.includes('twitch.tv/') ||
        u.includes('douyu.com/') ||
        u.includes('huya.com/') ||
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
    ...
    case "open_with_qlp":
        ...
        if (url !== "") {
            chrome.tabs.create({url: 'qliveplayer://' + url , active: true});
        }
        break;
    }
});
