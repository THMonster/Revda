...
void DanmakuLauncher::initPlayer()
{
    ...
    player_args.append(QString("curl '%1' -s -H 'User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36' | ffmpeg -i - -i '%2' -c copy -f matroska - | mpv -").arg(stream_url).arg(fifo));
...
int DanmakuLauncher::getDankamuDisplayLength(QString dm, int fontsize)
{
    ...
    return (fontsize * dm.size()) - (fontsize / 2 * ascii_num);
}
...
void DanmakuLauncher::launchDanmaku()
{
    ...
    while (!danmaku_queue.isEmpty()) {
        ...
        if (avail_dc >= 0) {
            ...
            QByteArray tmp;
            ass_event = QString("%4,0,R2L,,20,20,2,,{\\an7\\move(1920,%1,%2,%1)}%3").arg(QString().number(avail_dc*(font_size+10))).arg(QString().number(0-display_length)).arg(dm).arg(QString().number(read_order));
            ++read_order;
...
void DanmakuLauncher::launchVoidDanmaku()
{
    ...
    quint64 buf;
    QByteArray tmp;
    ass_event = QString("%1,0,R2L,,20,20,2,,{\\alphaff\\pos(1,1)}null").arg(QString().number(read_order));
    ++read_order;
    tmp = ass_event.toLocal8Bit();
    tmp.prepend((char)0x00);
    tmp.prepend((char)0x00);
...
int DanmakuLauncher::getAvailDanmakuChannel(double speed)
{
    ...
    {
        if (((double)(danmaku_channel[i].duration - c_pts + danmaku_channel[i].begin_pts) * speed) > 1080) {
            continue;
        } else {
            return i;
        }
